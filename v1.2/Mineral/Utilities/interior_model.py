...
# Define PerpleX_Error
class PerpleX_Warning(Exception):
    pass
...
def call_mineralogy_calculation(
    ...
    try:
        ...
        if not os.path.isfile(path + "/tmp_files/" + subpath_name + "/mantlephys_1.tab"):
            ...
            raise PerpleX_Warning(
                "Perple_X can not find a stable mineralogy for the given input. Pass."
            )
    ...
    except PerpleX_Warning as e:
        print(f"PerpleX warning: {e}")
        os.system("rm -rf " + path + "/tmp_files/" + subpath_name)
        return None, None  # Skip this simulation
...
class M_Planet:
    def __init__(self, M, CMF, mantle_chemsys, core_chemsys):
        ...
        if ~np.isnan(self.CMF) and self.CMF != 0.0:
    ...
    def _check_input(self, verbose=False):
        ...
        if total_mantle_mass !=0 and abs(1 - total_mantle_mass) > 0.001:
            if verbose:
                warnings.warn(
        ...
        if ~np.isnan(self.CMF) and self.CMF != 0:
            ...
            if total_core_mass !=0 and abs(1 - total_core_mass) > 0.001:
                if verbose:
                    warnings.warn(
    ...
    def init_planet(self):
        ...
        if ~np.isnan(self.CMF) and self.MMF != 0:
    ...
    def Msolve_structure(self, mantle_dict=None, subpath_name=None):
        ...
        ##>>to be double checked lines----
        self.successful = True
        self.error_flag = "No error occurd"
        ##--------------------------------
        ...
        #breakpoint()
        return self.R_mantle[-1], self.CRF
    ...
    def step(
        ...
        """
        if ~np.isnan(self.MMF) and self.MMF != 0:
            self.P_mantle = update_pressure(
                self.M_mantle, self.R_mantle, self.RHO_mantle, self.P_surface
            )
        if ~np.isnan(self.CMF) and self.CMF != 0:
...
class Planet(M_Planet):
    ...
    def __foo(self, M):
        self.M = M * Mearth
        #breakpoint()
        _R, self.CRF = self.Msolve_structure(mantle_dict=self.mantle_dict)
        return _R - self.R
    ...
    def solve_structure(self, subpath_name=None, M_low=0.001, M_max=10):
        ...
        if self.CMF < 1:
            ...
            if mantle_dict is not None:
                ...
                self.mantle_dict = None
                self.error_flag = "The planet has mantle, and but its composition is not validly computed."
        
        else:
            self.mantle_dict = None
            self.error_flag = "The planet has no mantle."

        self.M = brentq(self.__foo, M_low, M_max)
        if self.mantle_dict is not None:
        ...
        if self.M < 0.8 or self.M > 2:
            print("Please note that the default approach -- NL20 -- of calculating Delta_T_CMB has been replaced by stx14, since the mass of " + str(round(self.M, 3)) + " has been out of the parameterised range [0.8, 2]M_Earth of NL20!")
