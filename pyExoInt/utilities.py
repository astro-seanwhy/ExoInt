...
def load_data(filename, asymerr=False):
    ...
    stardex = np.asarray(tmp_table["abu"])
    stardexerr = np.asarray(tmp_table["err"])
        
    if asymerr == True:
        stardexerrdn = np.asarray(tmp_table["errdn"])
    else:
...
def produce_planetabuN(stardex, stardexerr, stardexerrdn, refsolar='A21'):
    ...
    if refsolar == 'A21':
        ...
        raise NotImplementedError("Invalid keyword for the reference solar abundance 'refsolar'; please choose one from 'A09'- Asplund et al. 2009, 'l09' - Lodders et al. 2009, and 'W19' - Wang et al. 2019; Or otherwise leave out the keyword, and the default (recommended) Asplund et al. 2021 solar abundance will be adopted.")
