...
def calculate_mineralogy(mantle_chemsys, path_name, verbose=False):
    ...
    abundances = str(mantle_chemsys.get('SiO2')) + ' ' + str(mantle_chemsys.get('MgO')) + ' ' \
             ...
             + ' ' + str(mantle_chemsys.get('CaO'))+ ' ' + str(mantle_chemsys.get('Na2O')) ##+ ' ' + str(mantle_chemsys.get('TiO2')) -- No EoS data for TiO2 in the stx11ver.dat of this Perple_X version.
    ...
    assert (len(filename)<200), "Path to Perple_X must not have more than 200 characters!"  
    ...
    p = pe.spawn(PerPlex_path+"/./werami", timeout=60)
    ...
    for phase in phases:
        ...
        try:
            ii = p.expect([pe.EOF, 'try again:','proportions keyword'], timeout=60)
            print('ii: ', ii)
        except pe.TIMEOUT:
            print(f"Timeout while waiting for response after sending phase '{phase}'. No worries -- Calculations are saved.")
            break

        if ii == 0: #print(f"EOF encountered after sending phase '{phase}'. Continuing with next phase.")
            continue
            #sys.exit()
        elif phase != 'seif' and ii == 1:  ##print(f"'try again' prompt received after sending phase '{phase}'. Retrying...")
            p.sendline('7')
            continue
        else:
            continue
    ...
    p.sendline('0')
    p.sendline('0')
    p.expect([pe.EOF, '0 - EXIT'], timeout=60)
    #p.sendline('0')
